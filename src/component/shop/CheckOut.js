...
import { placeOrderWithCT } from "../../actions/Cart";
...
class CheckOut extends Component {
  ...
  constructor(props) {
    ...
    this.props.getDealer(78207, this.getItemsSKUInfo());
  }
    ...
    this.setState(
      {
        fieldvalue: {
          ...
          spState: this.state.fieldvalue.state,
  ...
  placeOrderCall() {
    ...
    const paramData = this.getPlaceOrderParams();
    placeOrderWithCT(paramData.data).then((response) => {
      if (response) {
  ...
  getItemsSKUInfo() {
    const cart = JSON.parse(localStorage.getItem("LocalCartItems"));
    let skuItems = []
    if(cart){
      cart.forEach((item) => {
        skuItems.push({
  ...
  getCTParams() {
    ...
    getFulfillment() {
      return {
        ...
        payment_mode:
          this.state.selectedPaymentOption == "Online Payment" ? 1 : 0,
      };
  }
    ...
    let params = {
      data: {
        ...
        billing_address: {
          ...
          streetNumber: this.state.fieldvalue.line_1,
          streetName: this.state.fieldvalue.line_2,
          city: this.state.fieldvalue.city,
          state: this.state.fieldvalue.state,
          postalCode: this.state.fieldvalue.postcode,
          country: this.state.fieldvalue.country,
          mobile: this.state.fieldvalue.mobile_number,
          email: this.state.fieldvalue.email,
        },
        shipping_address: {
          firstName: this.state.fieldvalue.spFirstname,
          lastName: this.state.fieldvalue.spLastname,
          streetNumber: this.state.fieldvalue.spLine_1,
          streetName: this.state.fieldvalue.spLine_2,
          city: this.state.fieldvalue.spCity,
          state: this.state.fieldvalue.spState,
          postalCode: this.state.fieldvalue.spPostcode,
        ...
        <div class="shipping-fields__field-wrapper">
          ...
          <div className="form-group">
            ...
            <Input
                ...
                id="shipping_company"
                placeholder=""
                value={this.state.fieldvalue.spState}
                onChange={this.handleChange.bind(this, "spState")}
            />
          </div>
          <div class="form-group">
